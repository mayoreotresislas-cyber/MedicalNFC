...
// Centered and scaled down for the hexagon.
design_cy = 20.5;
front_cross_y = 17.5;
front_waves_y = 23.5;
back_phone_y = 20.3;
...
function hx() = body_w / 2;
function hy() = body_h / 2;
...
module front_art_2d() {
    union() {
        arc_text_top("SCAN ME", 10.7, 90, 2.85, design_cy, 90, 0.24);
        arc_text_bottom("MEDICAL INFO", 12.5, 270, 2.35, design_cy, 124, 0.17);
        front_waves_2d();
        medical_cross_2d();
        side_markers_2d();
    }
...
module phone_nfc_waves_2d() {
    translate([-1.5, 0.75])
        rotate(90)
            union() {
                rounded_arc_2d(1.55, 0.54, 214, 326, 6);
                rounded_arc_2d(2.65, 0.54, 214, 326, 6);
                rounded_arc_2d(3.75, 0.60, 214, 326, 6);
            }
}

module phone_nfc_label_2d() {
    translate([0, -4.2])
        thick_text("NFC", 1.8, 0.05);
}
...
module back_art_raw_2d() {
    union() {
        arc_text_top("TAP PHONE", 10.8, 90, 2.65, design_cy, 98, 0.24);
        arc_text_bottom("FOR MEDICAL INFO", 12.6, 270, 2.10, design_cy, 132, 0.17);
        phone_details_2d();
        side_markers_2d();
    }
}
