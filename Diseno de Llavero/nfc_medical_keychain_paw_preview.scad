...
// Compact layout to keep everything inside the main paw pad.
design_cy = 15.8;
front_cross_y = 12.7;
front_waves_y = 18.7;
back_phone_y = 15.1;
...
module front_art_2d() {
    union() {
        arc_text_top("SCAN ME", 8.4, 90, 2.55, design_cy, 92, 0.18);
...
module back_art_raw_2d() {
    union() {
        arc_text_top("TAP PHONE", 8.8, 90, 2.42, design_cy, 94, 0.18);
        arc_text_bottom("FOR MEDICAL INFO", 11.0, 270, 1.72, design_cy, 122, 0.12);
        phone_details_2d();
        side_markers_2d();
    }
}
